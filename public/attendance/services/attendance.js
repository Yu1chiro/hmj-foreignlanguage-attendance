...
function updateCountdown(timeData) {
    ...
    if (!countdownElement && headerElement) {
        ...
        countdownElement.className = 'text-center bg-blue-600 text-lg text-[#fafafafa] font-bold py-3 px-4 rounded-lg mb-5';
...
async function handleActiveSubmission(e) {
    ...
    if (!isValidNIM(nim)) {
        Swal.fire({
            ...
            <div class="flex justify-center">
             <img src="/Sources/logo.webp" style="width: 60px; height: 60px;" alt="Loading" class="mb-3 h-auto">
             </div>
    ...
    try {
        ...
        const nimRef = ref(database, `attendance-active/${nim}`, `attendance-late/${nim}`);
        ...
        if (!attendanceStatus.locationValid) {
            const confirmSubmit = await Swal.fire({
                html: `
    <img src="/Sources/logo.webp" alt="Logo" class="mb-4 h-16 w-16 mx-auto">
    ...
    <p class="text-base font-medium text-gray-600 mb-6">
        ...
        setTimeout(() => {
            ...
            Swal.fire({
                html: `
                    <div class="flex justify-center">
                        <img src="/Sources/logo.webp" style="width: 60px; height: 60px;" alt="Logo" class="mb-3 h-auto">
                    </div>
                ...
                preConfirm: () => {
                    ...
                    window.location.href = '/attendance/list/attendance-list.html';
                }
            });
...
async function saveActivePresence(form, position, attendanceStatus) {
    ...
    await set(ref(database, `attendance-active/${formData.nim}`), formData);
}
...
async function saveLatePresence(form) {
    ...
    await set(ref(database, `attendance-late/${formData.nim}`), formData);
}
